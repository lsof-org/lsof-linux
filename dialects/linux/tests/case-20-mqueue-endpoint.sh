...
{ $TARGET $mqname & } | {
    ...
    {
	{
	    ...
	    # mq-fork 18020 yamato    3u  PSXMQ   0,18       80     622461 /xxx 18021,mq-fork,3u
	    echo expected pattern: "mq-fork *$parent .* ${fd}u *PSXMQ .* $mqname $child,mq-fork,${fd}u"
	    $lsof +E -p "$parent" |
		grep -q "mq-fork *${parent} .* ${fd}u *PSXMQ .* ${mqname} ${child},mq-fork,${fd}u"
	} && {
	    echo From the parent side
	    # mq-fork 18021 yamato    3u  PSXMQ   0,18       80     622461 /xxx 18020,mq-fork,3u
	    echo expected pattern: "mq-fork *$child .* ${fd}u *PSXMQ .* $mqname $parent,mq-fork,${fd}u"
	    $lsof +E -p "$parent" |
		grep -q "mq-fork *${child} .* ${fd}u *PSXMQ .* ${mqname} ${parent},mq-fork,${fd}u"
	} && {
